...
Mann_Whitney_U_test_Fig_1A = mannwhitneyu(Epsilon_distributions_correlated_values[N_1e2_index],
                                          Epsilon_distributions_UN_correlated_values[N_1e2_index], nan_policy='omit')[1]

Mann_Whitney_U_test_Fig_1B = mannwhitneyu(Epsilon_distributions_correlated_values[N_1e4_index],
                                          Epsilon_distributions_UN_correlated_values[N_1e4_index], nan_policy='omit')[1]

Mann_Whitney_U_test_Fig_1C = mannwhitneyu(Epsilon_distributions_correlated_values[N_1e6_index],
                                          Epsilon_distributions_UN_correlated_values[N_1e6_index], nan_policy='omit')[1]

print ('Fig 1A: p value of Mann-Whitney test between correlated and uncorrelated molecular brightness distributions is: %s ' % Mann_Whitney_U_test_Fig_1A)
print ('Fig 1B: p value of Mann-Whitney test between correlated and uncorrelated molecular brightness distributions is: %s ' % Mann_Whitney_U_test_Fig_1B)
print ('Fig 1C: p value of Mann-Whitney test between correlated and uncorrelated molecular brightness distributions is: %s ' % Mann_Whitney_U_test_Fig_1C)


#%%

MEDIAN_I_mean_correlated = np.nanmedian(Mean_I_distributions_correlated_values, axis=1)
MAD_I_mean_correlated = median_abs_deviation(Mean_I_distributions_correlated_values, axis=1, nan_policy='omit')
MEDIAN_I_mean_UN_correlated = np.nanmedian(Mean_I_distributions_UN_correlated_values, axis=1)
MAD_I_mean_UN_correlated = median_abs_deviation(Mean_I_distributions_UN_correlated_values, axis=1, nan_policy='omit')


MEDIAN_VAR_correlated = np.nanmedian(Variance_distributions_correlated_values, axis=1)
MAD_VAR_correlated = median_abs_deviation(Variance_distributions_correlated_values, axis=1, nan_policy='omit')
MEDIAN_VAR_UN_correlated = np.nanmedian(Variance_distributions_UN_correlated_values, axis=1)
MAD_VAR_UN_correlated = median_abs_deviation(Variance_distributions_UN_correlated_values, axis=1, nan_policy='omit')


MEDIAN_epsilon_correlated = np.nanmedian(Epsilon_distributions_correlated_values, axis=1)
MAD_epsilon_correlated = median_abs_deviation(Epsilon_distributions_correlated_values, axis=1, nan_policy='omit')
MEDIAN_epsilon_UN_correlated = np.nanmedian(Epsilon_distributions_UN_correlated_values, axis=1)
MAD_epsilon_UN_correlated = median_abs_deviation(Epsilon_distributions_UN_correlated_values, axis=1, nan_policy='omit')
...
axs[0].text(0.85/tp, 3500, '$\#_{entries}= %s$' % str(len(Epsilon_distributions_UN_correlated_values[N_1e2_index])-
                         np.count_nonzero(np.isnan(Epsilon_distributions_UN_correlated_values[N_1e2_index]))), fontsize=9)
axs[1].text(0.85/tp, 350, '$\#_{entries}= %s$' % str(len(Epsilon_distributions_UN_correlated_values[N_1e4_index])-
                         np.count_nonzero(np.isnan(Epsilon_distributions_UN_correlated_values[N_1e4_index]))), fontsize=9)
axs[2].text(0.85/tp, 11.5, '$\#_{entries}= %s$' % str(len(Epsilon_distributions_UN_correlated_values[N_1e6_index])-
                         np.count_nonzero(np.isnan(Epsilon_distributions_UN_correlated_values[N_1e6_index]))), fontsize=9)
...
for i, ax in zip([N_1e2_index,N_1e4_index,N_1e6_index], axs.flatten()):
    ...
    ax.hist(Epsilon_distributions_UN_correlated_values[i], bins=bins, log=log, density = density, color= 'olivedrab', alpha=0.5, edgecolor='None')
    ax.hist(Epsilon_distributions_correlated_values[i], bins=bins, log=log, density = density, color= 'white', hatch="//////", alpha=0.5, edgecolor='lightgreen')    
    ax.axvline(np.nanmedian(Epsilon_distributions_UN_correlated_values[i]), ls='-', color='olivedrab', lw=0.75*lw, alpha=0.75)
    ax.axvline(np.nanmedian(Epsilon_distributions_correlated_values[i]), ls='-', color='lightgreen', lw=lw)
...
plt.ylabel('                                Frequency', loc='center', fontsize = fontsize, labelpad = labelpad*5)
...
while d1 < len(Epsilon_distributions_correlated_values):
    if abs(MEDIAN_epsilon_correlated[d1]-Epsilon_simulated)<0.050*max(abs(MEDIAN_epsilon_correlated-Epsilon_simulated)):
...
delta_diferences_UN_corr_index = min(range(len(MAD_epsilon_UN_correlated)), key=lambda i: abs(MAD_epsilon_UN_correlated[i]-MAD_epsilon_correlated[delta_diferences_index]))    

print(MEDIAN_epsilon_correlated[delta_diferences_index],'\n',
...
fig, (ax0, ax1, ax2) = plt.subplots(3, 1, figsize=(3, 3), sharex=True, sharey=False, dpi=300)
...
ax1.fill_between(N,
                  ...
                  MEDIAN_VAR_correlated + MAD_VAR_correlated, 
                  color= 'none', hatch="//////", alpha=0.3, edgecolor='salmon', label= 'VAR correlated data')
ax1.fill_between(N,
...
ax2.fill_between(N,
                  ...
                  MEDIAN_epsilon_correlated + MAD_epsilon_correlated, 
                  color= 'none', hatch="//////", alpha=0.5, edgecolor='lightgreen', label= 'B correlated data')
ax2.fill_between(N,
                  MEDIAN_epsilon_UN_correlated - MAD_epsilon_UN_correlated,
                  MEDIAN_epsilon_UN_correlated + MAD_epsilon_UN_correlated, 
                  color='olivedrab', alpha=0.3, label ='B uncorrelated data')
...
# Top right arrow
arrowprops = dict(arrowstyle='->, head_length=%s, head_width=%s'%(head_length, head_width), facecolor='lightgreen', edgecolor='lightgreen')
ax2.annotate('', xy=(N[delta_diferences_index], (MEDIAN_epsilon_correlated + MAD_epsilon_correlated*0.05)[delta_diferences_index]),
              xytext=(N[delta_diferences_index], (MEDIAN_epsilon_correlated + MAD_epsilon_correlated)[delta_diferences_index]*1.65),
              arrowprops=arrowprops)

# Bottom right arrow
arrowprops = dict(arrowstyle='<-, head_length=%s, head_width=%s'%(head_length, head_width), facecolor='lightgreen', edgecolor='lightgreen')
ax2.annotate('', xy=(N[delta_diferences_index], 0.1),
              xytext=(N[delta_diferences_index], (MEDIAN_epsilon_correlated - MAD_epsilon_correlated)[delta_diferences_index]*1.15),
              arrowprops=arrowprops)

# Top left arrow
arrowprops = dict(arrowstyle='->, head_length=%s, head_width=%s'%(head_length, head_width), facecolor='olivedrab', edgecolor='olivedrab')
ax2.annotate('', xy=(N[delta_diferences_UN_corr_index], (MEDIAN_epsilon_correlated + MAD_epsilon_correlated*0.05)[delta_diferences_index]),
              xytext=(N[delta_diferences_UN_corr_index], (MEDIAN_epsilon_correlated + MAD_epsilon_correlated)[delta_diferences_index]*1.65),
              arrowprops=arrowprops)

# Bottom left arrow
arrowprops = dict(arrowstyle='<-, head_length=%s, head_width=%s'%(head_length, head_width), facecolor='olivedrab', edgecolor='olivedrab')
ax2.annotate('', xy=(N[delta_diferences_UN_corr_index], 0.1),
              xytext=(N[delta_diferences_UN_corr_index], (MEDIAN_epsilon_correlated - MAD_epsilon_correlated)[delta_diferences_index]*1.15),
              arrowprops=arrowprops)
...
ax2.ticklabel_format(style='sci', axis='y', scilimits=(6, 6))  # Apply scientific notation to y-axis
